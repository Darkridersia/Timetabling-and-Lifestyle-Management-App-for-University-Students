  ...
  const pickImage = async () => {
    ...
    launchImageLibrary(options, async (response) => {
      ...
      if (response.assets && response.assets.length > 0) {
        ...
        const imageName =
          response.assets[0].fileName || `image_${Date.now()}.jpg`; // Fallback in case filename is missing
  ...
  const uploadImageToStorage = async (imageUri: string, imageName: string) => {
    try {
      ...
      if (Platform.OS === "android" && imageUri.startsWith("content://")) {
        ...
        const fileExtension = imageUri.split(".").pop() || "jpg"; // Default to jpg if no extension found
